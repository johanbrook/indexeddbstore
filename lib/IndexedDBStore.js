(function() {
	...
	function getStore(name, mode) {
		var defer = Q.defer();
		return connect(name).then(function(db) {
	...
	IndexedDBStore.prototype = {
		...
		all: function() {
			return getStore(this.name).then(function(store) {
				var records = [];
		...
		save: function(guid, blob) {
			...
			function saveToDb(data, guid) {
				return getStore(this.name, 'readwrite').then(function(store) {
					// Use GUID for key
			...
			function toJSONAndSave(guid, blob) {
				return Utils.blobToJSON(blob).then(function(obj) {
					return saveToDb.call(this, obj, guid);
				}.bind(this));
			}
			...
			if(Array.isArray(blob)) {
				return Q.all( blob.map(function(b) {
					...
					return toJSONAndSave.call(this, Utils.guid(), b);
				}.bind(this)));
			}
			else {
				guid = (arguments.length === 1) ? Utils.guid() : guid;
				return toJSONAndSave.call(this, guid, blob);
			}
		},
		...
		getAsString: function(guid) {
			return this.get(guid).then(function(record) {
